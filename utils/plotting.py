...
def centroid_visualization(test_data):
    ...
    plt.savefig("plots/centroid.png")  # Save the image after displaying the data
...
def visualize_number(data_of_image):
    ...
    x_index = 0
    y_index = 0
    for idx, curr_image in enumerate(data_of_image):
        number_matrix = []
        it = 0
        for i in range(8):
        ...
        if idx == 9:
            plt.savefig("plots/digits.png")  # final image
        y_index += 1
        if y_index > 2:
            y_index = 0
            x_index += 1
